...
## 🧪 Эксперименты по отказоустойчивости (наглядно продемострировано преподавателю)

**Эксперимент 1. Consumer отключен**

Действие: Остановили Consumer (Ctrl+C), Producer отправил 3 сообщения.

Результат: В очереди RabbitMQ накопилось 3 сообщения (Messages = 3, Consumers = 0).

Вывод: Сообщения не потерялись. При запуске Consumer'а они будут обработаны.

**Эксперимент 2. gRPC сервер отключен**

Действие: Остановили gRPC сервер, Producer отправил сообщение.

Результат: Consumer получил ошибку подключения, сообщение осталось в очереди.

Вывод: Отказ gRPC сервера не приводит к потере сообщений. После восстановления сервера сообщение обработается.

**Эксперимент 3. Полное восстановление**

Действие: Остановили всё, отправили 5 сообщений, запустили сервер и Consumer.

Результат: Consumer обработал все 5 накопленных сообщений.

Вывод: Система полностью восстанавливается после любых отказов компонентов.

---

# Заключение 

- Асинхронное взаимодействие через брокер сообщений обеспечивает слабую связанность сервисов
- Сообщения не теряются при отключении Consumer
- Система восстанавливается после отказов компонентов
- Docker упрощает развертывание инфраструктуры
